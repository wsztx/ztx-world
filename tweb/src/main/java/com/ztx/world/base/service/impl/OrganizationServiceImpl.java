...
public class OrganizationServiceImpl implements OrganizationService {
	...
	public Long saveOrganization(OrganizationVo organization) {
		...
		if(StringUtils.isEmpty(organization.getOrgCode())){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构编码不能为空.");
		}
		if(StringUtils.isEmpty(organization.getOrgName())){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构名称不能为空.");
		}
		if(organization.getParentId() == null){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构上级机构不能为空.");
		}
		...
		if(count >= 1){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构编码已存在.");
		}
		...
		if(organization.getId() == null){
			throw new BasicException(ResultEnum.BASE_DATA_ERROR, "新增机构失败.");
		}
	...
	public Long updateOrganization(OrganizationVo organization) {
		...
		if(organization.getId() == null){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构不存在.");
		}
		if(StringUtils.isEmpty(organization.getOrgCode())){
			throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构编码不能为空.");
		}
	...
	public void deleteOrganization(List<Long> ids) {
		if(!CollectionUtils.isEmpty(ids)){
			...
			if(count > 0){
				throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构下存在用户,无法删除.");
			}
			...
			if(dcount > 0){
				throw new BasicException(ResultEnum.BASE_ARG_ERROR, "机构下存在部门,无法删除.");
			}
